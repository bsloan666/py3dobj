...
def spur(radius, axle_radius, depth, twist):
    points1, indices1 = mach.segmented_spur(radius, 3, depth, twist, 1)
...
def bevel_spur(radius, axle_radius, depth, twist):
    points1, indices1 = mach.segmented_spur(radius, 3, depth, twist, (radius - depth)/radius)
    points2, indices2 = prim.conical_bushing(radius - 1, radius - (depth + 1), axle_radius, depth, 64)
    points1, indices1 = xfm.merge(points1, indices1, points2, indices2)
...
def ring(radius, depth, twist):
    points1, indices1 = mach.segmented_internal(radius, 3, depth, twist) 
